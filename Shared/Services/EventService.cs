...
namespace Shared.Services
{
    public class EventService : IEventService
    {
        ...
        private Dictionary<Guid, List<(MouseClickCallbackEventArgs args, Action<MouseClickEventArgs> callback)>> mouseClickReleaseEvents;
            ...
            Func<IApplicationInstance> getActiveApplication)
        {
            ...
            this.mouseClickReleaseEvents = new Dictionary<Guid, List<(MouseClickCallbackEventArgs args, Action<MouseClickEventArgs> callback)>>();
        ...
        private void InitializeMouseClickListener(Window window)
        {
            ...
            window.MouseButtonReleased += (_, e) =>
            {
                ...
                if (activeScreen is StackedScreen stackedScreen)
                {
                    foreach (var screen in stackedScreen.ReversedScreens)
                    {
                        ProcessMouseClickReleaseEvent(screen.Id, e, mouseEventArgs);
                ...
                {
                    ProcessMouseClickReleaseEvent(activeScreen.Id, e, mouseEventArgs);
                }
            };
        }
        ...
        private void ProcessMouseClickReleaseEvent(Guid screenId, MouseButtonEventArgs e, MouseClickEventArgs mouseEventArgs)
        {
            // There are no events set up for this screen
            if (!mouseClickReleaseEvents.ContainsKey(screenId))
            {
                return;
            }

            // There are events, and this screen is active, loop through each of its events and call them
            foreach (var mouseEvent in mouseClickReleaseEvents[screenId])
            {
                // If the event does not match the event arguments
                if (e.Button != mouseEvent.args.Button)
                {
        ...
        public void RegisterMouseClickReleaseCallback(
            Guid screenId,
            MouseClickCallbackEventArgs eventArgs,
            Action<MouseClickEventArgs> callback)
        {
            if (!this.mouseClickReleaseEvents.ContainsKey(screenId))
            {
                this.mouseClickReleaseEvents.Add(screenId, new List<(MouseClickCallbackEventArgs, Action<MouseClickEventArgs>)>());
            }

            this.mouseClickReleaseEvents[screenId].Add((eventArgs, callback));
        }
